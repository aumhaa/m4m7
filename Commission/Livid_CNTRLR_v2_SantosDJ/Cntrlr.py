...
class SantosMonoMixerComponent(MonoMixerComponent):


	def set_return_mute_buttons(self, buttons):
		for channel_strip, button in izip_longest(self._return_strips, buttons or []):
			if channel_strip:
				channel_strip.set_invert_mute_feedback(True)
				channel_strip.set_mute_button(button)
	...
	def _feedback_value(self, track, slot_or_clip):
		if self._clip_slot != None:
			...
			if slot_or_clip.color != None:
				if track.name == 'Deck A':
					return 'Session.DeckA_Stopped'
				elif track.name == 'Deck B':
					return 'Session.DeckB_Stopped'
				elif track.name == 'Deck C':
					return 'Session.DeckC_Stopped'
				elif track.name == 'Deck D':
					return 'Session.DeckD_Stopped'
				else:
...
class Cntrlr(Base_Cntrlr):
	...
	def _setup_mixer_control(self):
		self._mixer = SantosMonoMixerComponent(name = 'Mixer', num_returns = 4,tracks_provider = SessionRingComponent(num_tracks = 4, num_scenes = 4), track_assigner = simple_track_assigner, invert_mute_feedback = True, auto_name = True, enable_skinning = True)
	...
	def _setup_session_control(self):
		...
		self._session.clip_launch_layer = LayerMode(self._session, Layer(priority = 4,
									clip_launch_buttons = self._matrix.submatrix[:4,:],))
	...
	def _setup_modes(self):
		...
		self._mixer.santos_buttons_layer = AddLayerMode(self._mixer, Layer(priority = 5,
											arming_track_select_buttons = self._key_matrix.submatrix[:4, 1:],
