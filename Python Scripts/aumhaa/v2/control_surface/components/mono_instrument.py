...
from ableton.v2.base.util import forward_property, find_if, first, in_range, product

#from _Framework.SessionComponent import SessionComponent
from ableton.v2.control_surface.components.session import SessionComponent

#from _Framework.ClipCreator import ClipCreator
from ableton.v2.control_surface.clip_creator import ClipCreator
...
from pushbase.percussion_instrument_finder_component import PercussionInstrumentFinderComponent as DrumGroupFinderComponent
from pushbase.drum_group_component import DrumGroupComponent
...
#from aumhaa.v2.control_surface.components.original_drum_group import DrumGroupComponent
...
class MonoPlayheadComponent(PlayheadComponent):


	def set_clip(self, clip):
		super(MonoPlayheadComponent, self).set_clip(clip)
		debug('playhead set_clip:', clip)
...
class MonoStepSeqComponent(StepSeqComponent):
	...
	def __init__(self, clip_creator = None, skin = None, grid_resolution = None, note_editor_component = None, instrument_component = None, *a, **k):
		super(MonoStepSeqComponent, self).__init__(clip_creator = clip_creator, grid_resolution = grid_resolution, note_editor_component = note_editor_component, instrument_component = instrument_component, *a, **k)
		self._playhead_component = self.register_component(MonoPlayheadComponent(grid_resolution=grid_resolution, paginator=self._paginator, follower=self._loop_selector, notes=chain(*starmap(range, ((92, 100),
		 (84, 92),
		 (76, 84),
	...
	def set_playhead(self, playhead):
		super(MonoStepSeqComponent, self).set_playhead(playhead)
		debug('stepseq.set_playhead:', playhead)
...
class SplitModeSelector(ModeSelectorComponent):
	...
	def __init__(self, callback, *a, **k):
		super(SplitModeSelector, self).__init__(*a, **k)
		self._on_value = 127
...
class ScrollingOffsetComponent(ControlSurfaceComponent):
	...
	def __init__(self, callback, *a, **k):
		super(ScrollingOffsetComponent, self).__init__(*a, **k)
...
class ScaleSessionComponent(SessionComponent):
	...
	def __init__(self, session_ring, script, *a, **k):
		super(ScaleSessionComponent, self).__init__(session_ring, *a, **k)
		self._shifted = False
		self._script = script
...
class MonoInstrumentComponent(CompoundComponent):
	...
	def __init__(self, script, skin, grid_resolution, scalenames = SCALENAMES, *a, **k):
		...
		self._display = MonoScaleDisplayComponent(self, register_component = self.register_component, song = self.song)
		...
		self._split_mode_component = SplitModeSelector(self._split_mode_value, register_component = self.register_component, song = self.song)
		...
		self._sequencer_mode_component = SplitModeSelector(self._sequencer_mode_value, register_component = self.register_component, song = self.song)
		...
		self._vertical_offset_component = ScrollingOffsetComponent(self._vertical_offset_value, register_component = self.register_component, song = self.song)
		...
		self._offset_component = ScrollingOffsetComponent(self._offset_value, register_component = self.register_component, song = self.song)
		...
		self._drum_offset_component = ScrollingOffsetComponent(self._drum_offset_value, register_component = self.register_component, song = self.song)
		...
		self._scale_offset_component = ScrollingOffsetComponent(self._scale_offset_value, register_component = self.register_component, song = self.song)
		...
		self._keypad = MonoScaleComponent(parent = self, control_surface = script, skin = skin, grid_resolution = grid_resolution, is_enabled = False)

		self._drumpad = MonoDrumpadComponent(parent = self, control_surface = script, skin = skin, grid_resolution = grid_resolution, is_enabled = False)

		self.set_drumpad_mute_button = self._drumpad.set_mute_button
		self.set_drumpad_solo_button = self._drumpad.set_solo_button

		self._audio_loop = LoopSelectorComponent(follow_detail_clip=True, measure_length=1.0, name='Loop_Selector', register_component = self.register_component, song = self.song)
		...
		self._drum_group_finder = DrumGroupFinderComponent(device_parent=self.song.view.selected_track)
	...
	def _setup_selected_session_control(self):
		...
		self._selected_session = ScaleSessionComponent(self._session_ring, self, enable_skinning = True)
...
class MonoScaleComponent(CompoundComponent):
	...
	def __init__(self, parent, control_surface, skin, grid_resolution, *a, **k):
		...
		self._note_sequencer = MonoStepSeqComponent(clip_creator=scale_clip_creator, skin=skin, grid_resolution=grid_resolution, name='Note_Sequencer', note_editor_component=scale_note_editor, instrument_component=DrumGroupComponent(name = 'ScaleDrumGroupComponent') )
...
class MonoDrumpadComponent(CompoundComponent):
	...
	def __init__(self, parent, control_surface, skin, grid_resolution, *a, **k):
		...
		self._step_sequencer = MonoStepSeqComponent(clip_creator=drum_clip_creator, skin=skin, grid_resolution=grid_resolution, name='Drum_Sequencer', note_editor_component=drum_note_editor, instrument_component=DrumGroupComponent(name = 'DrumDrumGroupComponent') )
